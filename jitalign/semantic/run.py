...
from ..my_util import convert_examples_to_features, TextDataset, eval_result, create_path_if_not_exist, append_to_csv
...
def train(args, train_dataset, model, tokenizer):
    ...
    args.save_steps = len(train_dataloader) // 15  # evaluate 10 times each epoch
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss, prob = model(inputs_ids, attn_masks, labels)
            
            # logger.info('train loss {} & prob {}'.format(loss, prob))
            ...
            logger.info('处理后train loss {}'.format(loss))
            ...
            tr_num += 1
            if (step + 1) % 100 == 0:
                logger.info("epoch {} step {} loss {}".format(idx, step + 1, 
                                                              round(tr_loss / tr_num, 5)))
                writer.add_scalar("train/loss100", round(tr_loss / tr_num),  # 纵坐标的值
                      global_step  # 当前是第几次迭代，横坐标的值
                      )
                # 目前是每100步记录train loss最优的模型
                if round(tr_loss / tr_num, 5)<best_tr_loss:
                    best_loss = round(tr_loss / tr_num, 5)
                    checkpoint_prefix = 'checkpoint-best-tr_loss'
                    ...
                    logger.info("Saving best-tr_loss model checkpoint to %s", output_dir)
                tr_loss = 0
                tr_num = 0
            ...
            if (step + 1) % args.save_steps == 0:
               ...
               #            patience)
                writer.add_scalars("evaluate", 
                      {'loss': results['eval_loss'], 'precision': results['eval_precision'], 'recall': results['eval_recall'], 'f1':results['eval_f1']},
                      global_step 
                      )
                ...
                if results['eval_loss'] < best_eval_loss:
                    ...
                    logger.info("  " + "*" * 20)
                    logger.info("  Best eval_loss:%s", round(best_eval_loss, 4))
                    logger.info("  " + "*" * 20)
                    
                    checkpoint_prefix = 'checkpoint-best-eval_loss'
                    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model
                    ...
                    torch.save({
                        ...
                        'optimizer_state_dict': optimizer.state_dict(),
                        'scheduler': scheduler.state_dict()}, output_dir)
                    logger.info("Saving best_eval_loss model checkpoint to %s", output_dir)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    if os.path.exists(cache_dataset):
        ...
        eval_dataset = TextDataset(tokenizer, args, file_path=args.eval_data_file[0], mode='valid')
    ...
    print(y_trues, y_trues.tolist().count(0), len(y_trues), ' ', y_preds, len(y_preds), y_preds.tolist().count(True))
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    if os.path.exists(cache_dataset):
        eval_dataset = pickle.load(open(cache_dataset, 'rb'))
        logger.info("***** Test Data From Cache *****")
    else:
        # print('test_data_file', args.test_data_file)
    ...
    pd.set_option('display.max_columns', 10, 'display.max_rows', 10,'display.float_format', lambda x: '%.2f' %x) # 取消科学计数法
    # print(logits.shape)
    # print(logits)
    label_ = [t for y_true in y_trues for t in y_true]
    pred_ = [l[-1] for logit in logits for l in logit]
    df = pd.DataFrame({'label': label_, 
                       'pred': pred_})
    # print(os.getcwd())
    df.to_csv('.//semantic/pred_scores/test_com_' + args.test_data_file.split('/')[3] + '.csv', index=False, sep=',')
    
    auc = roc_auc_score(y_true=np.array(label_),  y_score=np.array(pred_))
    ...
    result_list = [round(result[key], 4) for key in result]
    header = ["eval_auc", "eval_recall", "eval_precision", "eval_f1", "eval_mcc", "eval_threshold", "eval_loss"]
   
    append_to_csv("test_score.csv", result_list, header)
    logger.info("test result has saved!")
    ...
    for example, pred, prob in zip(eval_dataset.examples, y_preds, logits[:, -1]):
        ...
        result[0].append(example.commit_id)
        result[1].append(prob)
        result[2].append(pred)
        result[3].append(example.label)
    RF_result = pd.DataFrame({'commit_id':result[0], 'prob':result[1], 'pred':result[2], 
                             'label':result[3]})
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--train_data_file", nargs=1, type=str, required=True,
                        help="The input training data file (a text file).")
    parser.add_argument("--output_dir", default=None, type=str, required=True,
                        help="The output directory where the model predictions and checkpoints will be written.")

    ## Other parameters
    parser.add_argument("--eval_data_file", nargs=1, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", nargs='?', type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    parser.add_argument('--no_abstraction', type=bool, default=True,
                        help='code process or not')
    parser.add_argument('--code_sequence_mode', 
                    choices=['random', 'file_origin', 'file_extension_sort', 'file_semantic_sort', 'file_extension_semantic_sort', 'code_semantic_sort', 'hunk_semantic_sort', 'msg_generation_sort', 'vector_sort'],
                    default='random',
                    help="Specify the file sorting mode")
    parser.add_argument('--code_sequence_settings', 
                    choices=['ascending', 'descending'],
                    default='ascending',
                    help="Specify the file sorting settings")
    ...
    logger.info("finish load tokenizer!")
    ...
    # codet5
    model = T5EncoderModel.from_pretrained(args.model_name_or_path, config=config)
    ...
    if args.do_train:
        ...
        if os.path.exists(cache_dataset):
            ...
            train_dataset = TextDataset(tokenizer, args, file_path=args.train_data_file[0])
